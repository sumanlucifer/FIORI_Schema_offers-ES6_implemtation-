sap.ui.define([
    ...
    return BaseController.extend("com.knpl.pragati.groupnotifications.controller.Object", {
        ...
        onInit: function () {
            ...
            var iOriginalBusyDelay,
                oViewModel = new JSONModel({
                    busy: true,
                    delay: 0,
                    TableList1Title:"Notification Group Members (0)",
                    TargetDetails: {
        ...
        _onObjectMatched: function (oEvent) {
            ...
            var sPath = "/NotificationGroupSet(" + sObjectId + ")"
            this._initData();
            this._getPainterData(sPath);
        },
        _getPainterData: function (sObjectPath) {
            var oView = this.getView();
            var othat = this;
            var oTable = oView.byId("List1");
            oTable.bindElement({
                path: sObjectPath,
                parameters: {
                    expand: "Members"
        ...
        onPainterUpdateFinished: function (oEvent) {
            var sTitle,
                oTable = oEvent.getSource(),
                iTotalItems = oEvent.getParameter("total");
            //var oViewModel = this.getView().getModel("objectView")
            if (iTotalItems && oTable.getBinding("items").isLengthFinal()) {
                sTitle = this.getResourceBundle().getText("NotificationGroupMembers", [
                    iTotalItems,
                ]);
            } else {
                sTitle = this.getResourceBundle().getText("NotificationGroupMembers", [0]);
            }
            
            this.getView()
            .getModel("objectView")
            .setProperty("/TableList1Title", sTitle);
        ...
        onMultyZoneSet: function () {
            ...
            this._fnChangeDivDepot({
                src: {
                    path: "/TargetDetails/NotificationGroupZone"
                },
                target: {
                    localPath: "/TargetDetails/NotificationGroupDivision",
            ...
            this._fnChangeDivDepot({
                src: {
                    path: "/TargetDetails/NotificationGroupDivision"
                },
                target: {
                    localPath: "/TargetDetails/NotificationGroupDepot",
