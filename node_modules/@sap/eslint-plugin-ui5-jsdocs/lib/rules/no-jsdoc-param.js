...
const getUndefinedModuleArgument = (aRelevantModuleArguments, oJsDocComment) => {
  ...
  return _.filter(aRelevantModuleArguments, oModuleArgument => {
    return _.isUndefined(
      _.find(aParamTags, oParamTag => {
        return oParamTag.name === oModuleArgument.argName;
      })
    );
  });
};

const reportJsDocParamUndefined = (
  context,
  oJsDocComment,
  ...
  _.forEach(aUndefinedModuleArguments, oUndefinedModuleArgument => {
    context.report({
      node: oUndefinedModuleArgument.moduleNode,
      message: messages.msg("no-jsdoc-param"),
      fix: fixer => {
        const sJsDocParam = helper.createJsDocParamWithNewLine(oUndefinedModuleArgument);
...
module.exports.meta = {
  type: "suggestion",
  docs: {
    description: "validates jsdoc param tag exists",
